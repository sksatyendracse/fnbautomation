...
// v.1.12 translations
$txt_city_edited    = !isset($txt_city_edited   ) ? "City edited successfully"    : $txt_city_edited;
$txt_country_edited = !isset($txt_country_edited) ? "Country edited successfully" : $txt_country_edited;
$txt_edit_city      = !isset($txt_edit_city     ) ? "Edit city"                   : $txt_edit_city;
$txt_edit_location  = !isset($txt_edit_location ) ? "Edit location"               : $txt_edit_location;
$txt_lat            = !isset($txt_lat           ) ? "Latitude"                    : $txt_lat;
$txt_lng            = !isset($txt_lng           ) ? "Longitude"                   : $txt_lng;
$txt_select_state   = !isset($txt_select_state  ) ? "Select State"                : $txt_select_state;
$txt_state_edited   = !isset($txt_state_edited  ) ? "State edited successfully"   : $txt_state_edited;
$txt_select_country = !isset($txt_select_country) ? "Select country"              : $txt_select_country;
$txt_state_abbr     = !isset($txt_state_abbr    ) ? "State abbreviation"          : $txt_state_abbr;
?>
<!DOCTYPE html>
							...
							<tr id="tr-city-<?= $city_id ?>">
								...
								<td class="nowrap">
									...
									<span data-toggle="tooltip" title="<?= $txt_edit_city ?>">
										<button class="btn btn-default btn-less-padding"
											data-loc-id="<?= $city_id ?>"
											data-loc-type="city"
											data-toggle="modal"
					...
					<table class="table responsive nowrap admin-table">
						...
						foreach($states_arr as $k => $v) {
							...
							<tr id="tr-state-<?= $state_id ?>">
								...
								<td>
									<!-- edit state -->
									<span data-toggle="tooltip" title="<?= $txt_edit_location ?>">
										<button class="btn btn-default btn-less-padding"
		...
		<div class="modal-content">
			<form class="form-create-loc" method="post">
				...
				<div class="modal-footer">
					...
					<a href="#" class="btn btn-less-padding btn-default" id="modal-create-cancel" data-dismiss="modal"><?= $txt_cancel ?></a>
				</div>
			</form>
		</div>
	</div>
...
<!-- edit loc modal -->
<div class="modal fade" id="edit-loc-modal" tabindex="-1" role="dialog" aria-labelledby="Edit Location Modal">
	<div class="modal-dialog" role="document">
		<div class="modal-content">
			<form class="form-edit-loc" method="post">
				<div class="modal-header">
					<h5 class="modal-title"><?= $txt_edit_location ?></h5>
				...
				<div class="modal-footer">
					<input class="btn btn-less-padding btn-blue" type="submit" id="edit-loc-submit">
					<a href="#" class="btn btn-less-padding btn-default" id="modal-edit-cancel" data-dismiss="modal"><?= $txt_cancel ?></a>
				</div>
			</form>
		</div>
	</div>
...
$(document).ready(function(){
	...
	$('.form-create-loc').submit(function(event){
		...
		$.post(post_url, {
			...
			function(data) {
				...
				modal.find('#modal-create-cancel').empty().text('OK');
			}
		);
	});
});
...
$(document).ready(function(){
	$('#edit-loc-modal').on('show.bs.modal', function (event) {
		...
		var post_url = '<?= $baseurl ?>' + '/admin/admin-get-loc.php';
	...
	// form in edit loc modal submitted
	$('.form-edit-loc').submit(function(event){
		event.preventDefault();

		var modal = $('#edit-loc-modal');
		var post_url = '<?= $baseurl ?>' + '/admin/admin-process-edit-loc.php';
		...
		$.post(post_url, {
			...
			function(data) {
				...
				modal.find('#edit-cancel').empty().text('OK');
			}
		);
	});
