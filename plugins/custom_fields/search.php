...
/*--------------------------------------------------
Match query and Match relevance query
--------------------------------------------------*/
$query_match = '';

if(strlen($user_query) > 2) {
	$query_match = " AND MATCH(p.place_name, p.description) AGAINST(:q IN BOOLEAN MODE) ";
}

// start with generic value as relevance so it will not affect ORDER BY clause if there is no relevance col
$query_match_relevance = ', 1 as relevance ';

if(strlen($user_query) > 2) {
	$query_match_relevance = ", MATCH(p.place_name, p.description) AGAINST(:q1 IN BOOLEAN MODE) AS relevance ";
}
	...
	if(empty($in_str)) {
		...
		if($total_rows > 0) {
			...
			$query = "SELECT
				...
				$query_where
				GROUP BY p.place_id
				ORDER BY relevance DESC, p.feat DESC, p.submission_date DESC
...
if($total_rows > 0) {
	...
	// bind matches
	if(strlen($user_query) > 2) {
		$stmt->bindValue(':q', $user_query);
		$stmt->bindValue(':q1', $user_query);
	}
