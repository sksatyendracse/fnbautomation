...
while($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
	switch ($row['property']) {
		...
		case 'cfg_use_select2':
			$cfg_use_select2 = !empty($row['value']) ? 1 : 0;
			break;
...
$lang_folder = __DIR__ . '/../translations';

// new folder structure, set default language folder
if(file_exists(__DIR__ . '/../language/en') || is_dir(__DIR__ . '/../language/en')) {
	$lang_folder = __DIR__ . '/../language/en';
}

// if folder defined in $html_lang exists, then use it
if(file_exists(__DIR__ . '/../language/' . $html_lang) || is_dir(__DIR__ . '/../language/' . $html_lang)) {
	$lang_folder = __DIR__ . '/../language/' . $html_lang;
}
...
switch(basename($_SERVER['SCRIPT_NAME'])) {
	...
	case 'coupons.php':
		require_once($lang_folder . '/trans-coupons.php');
		break;

	case 'posts.php':
		require_once($lang_folder . '/trans-posts.php');
		break;
	...
	case 'my-coupons.php':
		require_once($lang_folder . '/user_translations/trans-my-coupons.php');
		break;
...
if(!isset($txt_my_coupons)) $txt_my_coupons = "My Coupons";
if(!isset($txt_loading   )) $txt_loading    = "Loading...";
if(!isset($txt_print     )) $txt_print      = "Print";
