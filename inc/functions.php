...
function show_menu($group, $ul = true) {
	...
	$stmt = $conn->prepare('SELECT page_id, page_title, page_slug FROM pages WHERE page_group = :page_group ORDER BY page_order');
...
function get_children($parent_id, $place_cat, $level, PDO $conn, $cats=[]) {
	...
	while($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
		...
		if($level == 0) {
			$checkSel = in_array($cat_id, $cats)?'selected':'';
			echo "<option value='$cat_id' $selected $checkSel><strong>$name</strong></option>";
			echo "\n";
			get_children($cat_id, $place_cat, $level+1, $conn, $cats);
		}

		else {
			$indent = str_repeat("-", (($level * 2) - 1));
			$checkSel = in_array($cat_id, $cats)?'selected':'';
			echo "<option value='$cat_id' $selected $checkSel><strong>$indent $name</strong></option>";
			echo "\n";
			get_children($cat_id, $place_cat, $level+1, $conn, $cats);
		}
	}
